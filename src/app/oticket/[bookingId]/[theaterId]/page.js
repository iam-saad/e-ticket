...
async function fetchTicketData(bookingId, theaterId) {
  try {
    const res = await axios.post(
      `${appConstants.base_url}v1/booking/ticketing/eTicketOffline?bookingId=${bookingId}&threaterId=${theaterId}`,
      {},
      {
        headers: {
...
export default async function Page({ params }) {
  const { bookingId, theaterId } = params;
  const { ticketData, error } = await fetchTicketData(bookingId, theaterId);
  const isMobile =
